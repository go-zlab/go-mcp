...
// TestOAuthHandler_RefreshToken_EmptyAccessToken tests that go-mcp properly parses
...
// but go-mcp itself just parses what it receives.
func TestOAuthHandler_RefreshToken_EmptyAccessToken(t *testing.T) {
	...
	// go-mcp doesn't validate empty tokens - it just parses the response
	// The integration layer (MCPTokenStore) is responsible for validation
	require.NoError(t, err, "go-mcp should successfully parse the response")
